...
export default function IncomePage() {
  ...
  const fetchTransactions = useCallback(async () => {
    ...
    try {
      ...
      const incomeTransactions = (Array.isArray(data) ? data : []).filter(
      ...
      <div className="relative z-10 w-full max-w-2xl mx-auto px-6 py-12" style={{ animation: 'fadeInUp 0.8s ease-out' }}>
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-semibold text-white tracking-tight">Income History</h1>
              <p className="text-sm text-neutral-500">All your income transactions</p>
            </div>
          </div>
          <Link
            href="/account/income/new"
            className="flex items-center gap-2 rounded-xl bg-emerald-500/15 border border-emerald-500/20 px-4 py-2.5 text-sm font-medium text-emerald-400 hover:bg-emerald-500/25 transition-all duration-300"
          >
        ...
        <div className="relative mb-8">
          <div className="absolute inset-0 rounded-2xl bg-gradient-to-b from-emerald-500/10 to-emerald-500/5 blur-xl" />
          <div className="relative rounded-2xl border border-emerald-500/20 bg-neutral-900/60 backdrop-blur-xl p-6 shadow-xl shadow-black/20">
            <p className="text-sm text-neutral-400 mb-1">Total Income</p>
            {loading ? (
              <div className="h-8 w-36 bg-neutral-800/50 rounded-lg animate-pulse" />
            ) : (
              <p className="text-3xl font-bold text-emerald-400">
                Bs. {totalIncome.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </p>
            )}
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            <div className="w-16 h-16 rounded-2xl bg-emerald-500/10 flex items-center justify-center mx-auto mb-4">
              <svg className="w-8 h-8 text-emerald-500/50" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M12 7a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0V8.414l-4.293 4.293a1 1 0 01-1.414 0L8 10.414l-4.293 4.293a1 1 0 01-1.414-1.414l5-5a1 1 0 011.414 0L11 10.586 14.586 7H12z" clipRule="evenodd" />
              </svg>
            </div>
            <p className="text-sm text-neutral-400">No income transactions yet</p>
          </div>
        ) : (
          <div className="space-y-3">
            {transactions.map((transaction) => (
              <div
                key={transaction.id}
                className="rounded-xl border border-neutral-800/50 bg-neutral-900/40 backdrop-blur-sm p-4 transition-all duration-300 hover:border-emerald-500/20 hover:bg-emerald-500/5"
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1">
                      <Link
                        href={`/account/income/edit/${transaction.id}`}
                        className="w-7 h-7 rounded-lg border border-neutral-700/50 bg-neutral-800/50 flex items-center justify-center hover:border-emerald-500/30 transition-colors"
                        title="Edit"
                      >
